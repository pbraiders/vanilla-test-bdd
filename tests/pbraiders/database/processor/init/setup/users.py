...
class Users(AbstractProcessor):
    ...
    def execute(self, config: dict):
        ...
        theUsers['admin']['password'] = hashlib.sha1(
            theUsers['admin']['password'].encode()).hexdigest()
        theUsers['simple']['password'] = hashlib.sha1(
            theUsers['simple']['password'].encode()).hexdigest()
        theUsers['disabled']['password'] = hashlib.sha1(
            theUsers['disabled']['password'].encode()).hexdigest()
