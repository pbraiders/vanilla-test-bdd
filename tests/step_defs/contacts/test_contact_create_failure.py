...
def send_data_without_lastname(page_new_contact, the_faker) -> None:
    ...
    page_new_contact.set_contact(ContactFakerFactory(_faker=the_faker).initialize(config={}))
    p_action = FillContactAction(_page=page_new_contact)
    p_action.fill_firstname().fill_phone()
    del p_action
    p_action = CreateContactAction(_page=page_new_contact)
    p_action.click()
